...
// import { upload} from '../models/db-model';
    ...
    if (!error) {
      ...
      Company.findOne({$and: [{name: req.body.company}, {accept: 1}]}).then(function (result) {
        getStatic(function (resul, parse) {
          ...
          //лого
          let writestream = dbModel.gfs.createWriteStream({
            filename: req.files['upload'][0].filename
          });
          dbModel.fs.createReadStream(req.files['upload'][0].path)
            .on('end', function () {
              dbModel.fs.unlink(req.files['upload'][0].path, function (err) {
                console.log("success")
              })
            })
            .on('err', function () {
          ...
          if (req.files['ru_PDF'] != undefined) {
            writestream = dbModel.gfs.createWriteStream({
              filename: req.files['ru_PDF'][0].filename
            });
            dbModel.fs.createReadStream(req.files['ru_PDF'][0].path)
              .on('end', function () {
                dbModel.fs.unlink(req.files['ru_PDF'][0].path, function (err) {
                  console.log("success")
                })
              })
              .on('err', function () {
          ...
          if (req.files['en_PDF'] != undefined) {
            ...
            dbModel.fs.createReadStream(req.files['en_PDF'][0].path)
              .on('end', function () {
                dbModel.fs.unlink(req.files['en_PDF'][0].path, function (err) {
                  console.log("success")
                })
              })
              .on('err', function () {
                console.log('Error uploading pdf_en')
              })
              .pipe(writestream);
            writestream.on('close', function (file) {
              //console.log(file.filename + ' Written To DB');
