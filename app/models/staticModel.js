...
let getStatic = function (callback) {
  staticModel.find({}, function (err, docs) {
    ...
    function parse(result) {
      ...
      for (i = 0; i < result.length; i++) {
        switch (result[i].name) {
          case 'standarts' :
            stat.standarts = result[i].mass;
            break;
          case 'sector':
            stat.sector = result[i].mass;
            break;
          case 'opf':
            stat.opf = result[i].mass;
            break;
          case 'size_of_company':
            stat.size_of_company = result[i].mass;
            break;
          case 'type_of_ownership':
            stat.type_of_ownership = result[i].mass;
            break;
          case 'year':
            stat.year = result[i].mass;
            break;
          case 'best':
            stat.best = result[i].mass;
            break;
          case 'city':
            stat.city = result[i].mass;
            break;
        }
      }
      return stat;
